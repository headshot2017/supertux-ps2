...
//      worldmap.set_levels_as_solved();
      ...
      CreateSave("save" + savegame, savegame);

      savegame = std::string(st_save_dir) + savegame + "/" + savegame + ".stsg";
...
void title(void)
{
  ...
  fadeout("Initializing title demo");
  ...
  string_list_add_item(&worldmap_list, "bonusisland1.stwm");
  string_list_add_item(&worldmap_list, "bonusisland2.stwm");
          ...
          else if(menu == load_game_menu)
            {
              if(event.key.keysym.sym == SDLK_DELETE)
                ...
                if(confirm_dialog(str))
                  {
                  sprintf(str,"%s%d/slot%d.stsg", st_save_dir, slot, slot);
                  printf("Removing: %s\n",str);
                  remove(str);
                  }
