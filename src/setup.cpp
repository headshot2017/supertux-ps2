...
bool process_load_game_menu()
{
  ...
  if(slot != -1 && load_game_menu->get_item_by_id(slot).kind == MN_ACTION)
    {
      char slotfile[1024];
      snprintf(slotfile, 1024, "%s%d/slot%d.stsg", st_save_dir, slot, slot);
      char slotname[32], slotdir[32];
      sprintf(slotdir, "save%d", slot);
      sprintf(slotname, "Slot %d", slot);
      CreateSave(slotdir, slotname);

      FILE* fd = fopen(slotfile, "r");
      if (!fd)
        {
          draw_intro();
        }
